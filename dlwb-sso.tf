resource "aws_ecs_task_definition" "dlwb_sso" {
  family                   = "dlwb-sso"
...
resource "aws_ecs_service" "dlwb_sso" {
  name            = "dlwb-sso"
  cluster         = aws_ecs_cluster.dlwb_fargate.id
  task_definition = aws_ecs_task_definition.dlwb_sso.arn
  desired_count   = 1
  launch_type     = "FARGATE"
  ...
  network_configuration {
    ...
    security_groups  = [aws_security_group.dlwb_sso_sg.id]
...
resource "aws_security_group" "dlwb_sso_sg" {
  name   = "dlwb_sso_sg"
